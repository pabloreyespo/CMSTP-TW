...
def gurobi_fast_solution(ins, time_limit = 1800, start = None, rando = False):
    ...
    if start:                
        for j in ins.nodes[1:]:
            ...
            # d[j].Start = start.arrival[j] # fijar salida en solución inicial
            if rando:
                xij = mdl.addVar(vtype = GRB.BINARY, lb=1, ub=1, name = "x[%d,%d]" % (i,j)) # rando contreras
                xij.VarHintVal = 1
                xij.VarHintPri = int(ins.maxcost * (1 + 1/ins.cost[i,j]))
...
def test(gurobi_prop, ils_prop, global_time, q, nnodes, ins_folder):
...
if __name__ == "__main__":
    argv = "-a 0.032 -f 10000 -e 20000 -r 6000 -s 20 -n 7.001 -x 0.121 -y 0.677 -z 0.186 -c 0.016 -u 0.013 -v 0.157 -w 0.830 -t 5"
    argv = argv.split()
    try:
        opts, args = getopt.getopt(argv, 'a:f:e:r:s:n:x:y:z:c:u:v:w:t:')
        print("Leido")
    ...
    # lsp1 = 0.013
    # lsp2 = 0.157
    # theta1, theta2, theta3 = pp1, pp1 + pp2, pp1 + pp2 + pp3
    # phi1, phi2 = lsp1, lsp1+lsp2
    # alpha, beta, gamma = 10000,20000,6000
    # mu = 0.032
    # e_size = 20
    # BRANCH_TIME = 5
    # INITIAL_TRIGGER = 40
    ...
    for q in capacities:
        ...
        gurobi_prop = 15
        ils_prop = 10
        nombre = f"MLM_proporciones_rando_gurobi_{gurobi_prop}_{ils_prop}_{global_time}_Q{q}_n{nnodes}"
