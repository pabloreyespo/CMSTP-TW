...
def algorithm(ins, b, alpha, s, mu, vis  = False, Q = None, initial = False):
    ...
    while len(nodes_left) > 0:
        ...
        cost += distance(kk,jj)
        gate[jj] = gate[kk] if gate[kk] != 0 else jj
        load[gate[jj]] += 1
...
def merge_branches(s):
    ...
    for i in range(nds//2):
        ...
        if lo < 5 and lo >= 2:
            ...
            aux = branch_gurobi(branch, parent)
        else:
            aux = branch_gurobi(branch, parent, initial = True)
    ...
    cost, feasible = fitness((parent, gate, load, arrival))
    return (parent, gate, load, arrival), cost, feasible   
...
def optimal_branch(s):
    ...
    for i in set(gate):
        if i != 0:
            lo = load[i]
            if lo <= 20 and lo >= 2:
                branch = [j for j in range(1, len(parent)) if gate[j] == i]
                if lo < 5 and lo >= 2:
                    bb = branch_bound(branch)
                    aux = bb.give_solution()
                else:
...
def best_father(s): #perturbation 1
    ...
    for j in nodes:
        ...
        for jj in sorted(disc, key = lambda x: arrival[x]):
            arrival[jj] = max(arrival[parent[jj]] + distance(jj, parent[jj]), earliest[jj])
            gate[jj] = subroot
        load[gate[j]] += l
...
def ILS_solution(ins, semilla = None, acceptance = 0.05,
    ...
    while get_counter() < limit:
        ...
        if (it + 1) % feasibility_param == 0:
            try: s = deepcopy(s_best_unfeasible)
            except: s = s
...
#     a, f, e, s, n, x, y, z, r, l, t, g = 0.019, 1000, 6000, 15, 2.554, 0.255, 0.577, 0.168, 7500, 0.177, 3, 30
...
# Podría ser una idea curiosa para el futuro hacer que la solución fuera una instancia, que conetnag como partes todo lo que actualmente existe ne estos codigos y que resuelva todo en forma más clara