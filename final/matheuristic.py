...
LOCAL_SEARCH_PARAM2 = 0.1 + 0.9 * 0.18 # best_father
BRANCH_TIME = 4
INITIAL_TRIGGER = 40
...
def branch_to_root(s):
    ...
    s.gate[j] = j
    s.parent[j] = 0
    s.arrival[j] = distance(0,j)
    if s.arrival[j] < earliest[j]:
        s.arrival[j] = earliest[j]

    # discover which nodes are part of the branch
    for i in sorted(range(n), key = lambda x: s.arrival[x]):
    ...
    for i in sorted(nodes, key = lambda x: s.arrival[x]): #this should avoid using j
        s.connect(0,i)
...
def branch_to_branch(s):
    ...
    for i in sorted(range(n), key = lambda x: random()):
        if i not in nodes:
            ...
            if s.load[g] + lo <= Q:
                s.parent[j] = i
                for j in sorted(nodes, key = lambda x: s.arrival[x]): #this should avoid using j
                    k = s.parent[j]
                    s.connect(k,j)
    return s

def best_father(s, times): #perturbation 1
    n = len(s.parent)
    tries = sample(range(1,n), times)
    ...
    for j in tries:
        ...
        con, disc = [i for i in range(n) if i not in nodes], nodes

        l = len(disc)
        s.load[s.gate[j]] -= l # descontar lo que està de sobra
        minim = inf
        for i in con:
            if s.load[s.gate[i]] + l <= Q and D[i,j] < minim:
                minim = D[(i,j)]
                k = i

        s.parent[j] = k
        for jj in sorted(disc, key = lambda x: s.arrival[x]):
            s.connect(s.parent[jj], jj)
    cost, feasible = s.fitness()
...
def ILS_solution(ins, semilla = None, acceptance = 0.033,
                feasibility_param = 9500, elite_param = 10000, elite_size = 40, iterMax = 15000, p = PENALIZATION,
                pp = PERTURBATION,  lsp1 = LOCAL_SEARCH_PARAM1, lsp2 = LOCAL_SEARCH_PARAM2,  initial_solution = None,
                elite_revision_param = 4000, vis  = False, verbose = False, time_limit = 60, limit_type = "t"):
...
def main():
    ...
    initial_solution = lambda x: (deepcopy(s), objective_value)

    obj, time, best_bound, gap = ILS_solution(
            ins, semilla = 1, initial_solution = initial_solution, #   initial_solution,
...
def test(gurobi_time, q, nnodes, ins_folder):
    if gurobi_time == 30:
        ...
        a, f, e, s, n, x, y, z, r, l, t = 0.026, 6000, 5000, 30, 9.378, 0.281, 0.392, 0.327, 1000, 0.402, 2
    else:
        a, f, e, s, n, x, y, z, r, l, t = 0.033 ,9500 ,10000 ,40 ,6.326 ,0.182 ,0.476 ,0.342 ,4000 ,0.18 ,4
    ...
    for p in instances:
        ...
        for i in range(10):
            obj, time, best_bound, gap = ILS_solution(
                ...
                feasibility_param = f, elite_param = e, elite_size = s, initial_solution = initial_solution,
                elite_revision_param = r, vis  = False, verbose = False, time_limit = 60 - gurobi_time)
            if obj < best_obj:
                best_obj = obj
            time_sum += time
            solution_sum += obj
...
# algo pasa en la perturbacion 3 que además no es detectado por la funcion fitness como infactibilidad

# separar las de local search y regenerar las perturbaciones 