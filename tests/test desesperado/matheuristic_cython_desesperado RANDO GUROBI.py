...
PENALIZATION = 7.001
PERTURBATION1 = 0.121
PERTURBATION2 = 0.677 + PERTURBATION1
PERTURBATION3 = 0.186 + PERTURBATION2
LOCAL_SEARCH_PARAM1 = 0.013
LOCAL_SEARCH_PARAM2 = 0.157 + LOCAL_SEARCH_PARAM1
BRANCH_TIME = 5
INITIAL_TRIGGER = 40
...
def ILS_solution(ins, semilla = None, acceptance = 0.032,
                feasibility_param = 10000, elite_param = 20000, elite_size = 20, iterMax = 15000, p = PENALIZATION,
                ...
                elite_revision_param = 6000, vis  = False, verbose = False, time_limit = 300, limit_type = "t"):
...
def main():
    ...
    print("prim:", cost)
    s, cost = gurobi_solution(ins, vis = False, time_limit= 20, verbose = False, initial=True, start = s)
    print("gurobi:", cost)
    ...
    obj, time, best_bound, gap = ILS_solution(
            ...
            vis  = True, verbose = False, time_limit = 40 + 60*4 )
...
def test(global_time, q, nnodes, ins_folder):
    ...
    for p in instances:
        ...
        s, cost = prim(ins, vis = False, initial = True)
        print("prim:", cost)
        s, cost, optimal = gurobi_solution(ins, vis = False, time_limit= gurobi_time, initial=True, start = s)
        print("gurobi:", cost)

        initial_s = deepcopy(s)
        initial_cost = cost
        initial_solv = lambda x: (deepcopy(initial_s), initial_cost)
        if not optimal:
            ...
            for seed in range(tries):
                ...
                time = perf_counter()
                s, cost = ILS_solution(
                    ins, semilla = seed, vis  = False, verbose = False, time_limit = global_time - gurobi_time, initial_solution=initial_solv)
                print("ILS:", cost)
...
def test_final(global_time, q, nnodes, ins_folder):
    if global_time < 100:
        global_time = 100
    ...
    for p in instances:
        ...
        if not optimal:
            ...
            for seed in range(tries):
                ...
                time = perf_counter()
                s, cost = ILS_solution(
                    ins, semilla = seed, vis  = False, verbose = False, time_limit = 100 - gurobi_time, initial_solution=initial_solv)
                print("ILS:", cost)
                for _ in range((global_time - 100)//100):
                    s, cost, optimal = gurobi_solution(ins, vis = False, time_limit= gurobi_time, initial=True, start = s, rando = True)
                    if optimal: 
                        break
                    print("gurobi:", cost)
                    initial_s = deepcopy(s)
                    initial_cost = cost
                    initial_ = lambda x: (deepcopy(initial_s), initial_cost)
                    
                    s, cost = ILS_solution(
                        ins, semilla = seed, vis  = False, verbose = False, time_limit = 100 - gurobi_time, initial_solution=initial_)
                    print("ILS:", cost)
...
if __name__ == "__main__":
    ...
    for q in capacities:
        ...
        nombre = f"MLM 60-40 RANDO GUROBI {global_time} Q-{q} n-{nnodes}"
        test_final(global_time, q, nnodes, ins_folder)