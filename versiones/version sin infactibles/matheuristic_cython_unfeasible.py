...
from operators_cython_unfeasible import perturbation, best_father, fitness
...
def merge_branches(s):
    ...
    cost, feasible = fitness(s, latest)
...
def local_search(s):
    ...
    if x < LOCAL_SEARCH_PARAM1:
        ...
        # print("LB")
        return best_father(s,1, latest)
    else:
        # print("LC")
        return best_father(s,5, latest)

def ILS_solution(ins, semilla = None, acceptance = 0.026, elite_param = 5000, elite_size = 30, iterMax = 15000,
    ...
    global Q, PERTURBATION1, PERTURBATION2,PERTURBATION3, LOCAL_SEARCH_PARAM1, LOCAL_SEARCH_PARAM2, D
    PERTURBATION1 = pp1
    PERTURBATION2 = pp2
    PERTURBATION3 = pp3
...
def test(gurobi_time, q, nnodes, ins_folder):
    global PERTURBATION1, PERTURBATION2,PERTURBATION3 ,LOCAL_SEARCH_PARAM1 ,LOCAL_SEARCH_PARAM2 ,BRANCH_TIME ,INITIAL_TRIGGER
    ...
    for p in instances:
        ...
        for i in range(10):
            obj, time, best_bound, gap = ILS_solution(
                ins, semilla = i, initial_solution = initial_solution,
                vis  = False, verbose = False, time_limit = 60 - gurobi_time)
            if obj < best_obj:
                best_obj = obj
            time_sum += time
            solution_sum += obj
